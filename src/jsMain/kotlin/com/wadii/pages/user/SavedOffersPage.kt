...
import com.wadii.viewmodel.SavedOffersEvent
import com.wadii.viewmodel.SavedOffersScreenModel
                ...
                if (offers.isEmpty()) {
                    ...
                    Div(attrs = { classes("grid", "grid-cols-1", "md:grid-cols-2", "gap-4") }) {
                        offers.forEach { offer ->
                            SavedOfferCard(offer,
                                onRemove = { model.onEvent(SavedOffersEvent.Remove(offer)) },
                                onProviderClick = { offer.provider?.let { navigator.push(ProviderDetailScreen(it.id)) } }
                            )
                        }
                    }
                }
...
@Composable
private fun SavedOfferCard(offer: Offer, onRemove: () -> Unit, onProviderClick: () -> Unit) {
