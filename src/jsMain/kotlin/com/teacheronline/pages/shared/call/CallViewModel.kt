...
import com.teacheronline.data.livekit.LiveKitCallClient
import com.teacheronline.data.livekit.LiveKitClient
        ...
        liveKitUseCase.getToken(
            ...
            r.handelState(
                onSuccess = { resp ->
                    ...
                    if (tokenData == null) {
                        updateState { it.copy(status = CallStatus.ENDED, error = "No LiveKit token returned") }
                    } else {
                        screenModelScope.launch(Dispatchers.Default) {
                            runCatching {
                                withContext(Dispatchers.Main) {
                                    ...
                                    val localTrack = liveKitClient.join(tokenData.url, tokenData.token, withVideo)
                                    updateState {
                                        it.copy(
