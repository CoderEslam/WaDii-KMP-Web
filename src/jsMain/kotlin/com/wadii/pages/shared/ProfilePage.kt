...
import com.wadii.viewmodel.ProfileEvent
import com.wadii.viewmodel.ProfileScreenModel
                ...
                Div(attrs = { classes("bg-white", "rounded-2xl", "shadow-sm", "overflow-hidden") }) {
                    Div(attrs = { classes("relative", "h-40", "bg-gradient-to-r", "from-amber-400", "to-amber-600") }) {
                        u.backgroundImage?.let { bg ->
                            Img(src = "http://192.168.1.30:8080/uploads/$bg", attrs = { classes("absolute", "inset-0", "w-full", "h-full", "object-cover") })
                        }
                        Button(attrs = {
