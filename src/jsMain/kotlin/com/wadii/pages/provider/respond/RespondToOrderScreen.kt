        ...
        Div(attrs = { classes("max-w-2xl", "mx-auto", "space-y-6") }) {
            ...
            when {
                ...
                state.order != null -> {
                    val o = state.order!!
                    Card(classes = "p-6") {
                        H1(attrs = { classes("text-xl", "font-semibold", "text-heading", "mb-4") }) { Text("Order #${o.id}") }
                        Div(attrs = { classes("space-y-2", "text-sm") }) {
                            P { Span(attrs = { classes("text-body-subtle") }) { Text("Vehicle: ") }; Span(attrs = { classes("font-medium", "text-heading") }) { Text(o.carModelYear) } }
                            P { Span(attrs = { classes("text-body-subtle") }) { Text("Description: ") }; Span(attrs = { classes("text-body") }) { Text(o.comment) } }
                            ...
                            o.spareParts?.takeIf { it.isNotEmpty() }?.let { parts ->
                                Div(attrs = { classes("mt-2") }) {
                                    P(attrs = { classes("text-body-subtle", "mb-1") }) { Text("Spare parts:") }
                                    parts.forEach { sp -> P(attrs = { classes("text-body") }) { Text("• ${sp.sparePartName}") } }
                                }
                            }
                        }
                    }
                    Card(classes = "p-6") {
                        ...
                        Div(attrs = { classes("space-y-5") }) {
                            ...
                            if (state.prices.isNotEmpty() && o.spareParts != null) {
                                Div {
                                    P(attrs = { classes("text-sm", "font-medium", "text-heading", "mb-2") }) { Text("Spare Parts Pricing") }
                                    Div(attrs = { classes("space-y-2") }) {
                                        o.spareParts!!.forEachIndexed { i, sp ->
                                            Div(attrs = { classes("flex", "items-center", "gap-3") }) {
                                                Span(attrs = { classes("flex-1", "text-sm", "text-body") }) { Text(sp.sparePartName) }
                                                Div(attrs = { classes("relative"); style { property("width", "128px") } }) {
                                                    Span(attrs = { classes("absolute", "left-3", "top-1/2", "-translate-y-1/2", "text-body-subtle", "text-sm", "pointer-events-none") }) { Text("$") }
                                                    Input(type = InputType.Text, attrs = {
                                                        classes(
                                                            "w-full", "pl-7", "pr-3", "py-2", "border", "border-default-medium", "rounded-neu-base",
                            ...
                            PrimaryButton("Submit Response", loading = state.submitting, fullWidth = true) {
